...
def test_catalog_contains_seeded_server_name(client: TestClient) -> None:
    ...
    resp = client.get("/")
    # seed_servers.json ships several; we know 'acme-search' is one slug.
    # Look for a known name that appears in the seed JSON.
    body = resp.text
    # The catalog table must contain a link to at least one server.
...
def test_catalog_shows_grade_after_scan(client: TestClient) -> None:
    # Trigger a scan.
    client.post("/servers/acme-search/scan")
    resp = client.get("/")
    body = resp.text
...
def test_detail_known_server_200(client: TestClient) -> None:
    resp = client.get("/ui/servers/acme-search")
    assert resp.status_code == 200
    assert "text/html" in resp.headers["content-type"]
...
def test_detail_shows_grade_after_scan(client: TestClient) -> None:
    ...
    scan_resp = client.post("/servers/acme-search/scan")
...
def test_detail_badge_snippet_slug_correct(client: TestClient) -> None:
    ...
    assert "/servers/acme-search/badge.json" in body
    assert "/ui/servers/acme-search" in body
