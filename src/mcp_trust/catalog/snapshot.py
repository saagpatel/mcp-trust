...
def build_snapshot(
    ...
    *,
    ...
    try:
        return build_snapshot_from_connection(
            conn,
            excluded_slugs=excluded_slugs,
            masked_slugs=masked_slugs,
            verified_scan_modes=verified_scan_modes,
            now=now,
        )
    finally:
        conn.close()
...
def build_snapshot_from_connection(
    ...
    *,
    ...
    """Build a snapshot from an already-open, caller-owned database image."""
    ...
    for server in sorted(server_repo.list(), key=lambda item: item.slug):
        ...
        verified_scan_mode = (
            verified_scan_modes.get(scan.id) if verified_scan_modes is not None else None
        )
        if (
            not remote_live
